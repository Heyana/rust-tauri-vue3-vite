...
const createItemRenderList = () => {
    ...
    console.log('Log-- ', 'createItemRenderList');
    ...
    console.log('Log-- ', [content.value], '[content.value]');
    // 避免多次访问
    const innerHeight = content.value.parentElement.clientHeight
    console.log('Log-- ', innerHeight, itemSpaces.value, 'innerHeight');
    ...
    console.log('Log-- ', minLimit, maxLimit, 'minLimit,maxLimit');
    ...
    console.log('Log-- ', start, end, 'start,end');
    if (lineStart == start && lineEnd === end) return

    lineStart = start

    lineEnd = end
    console.log('Log-- ', start, end + 1, 'start,end+1');

    // 测试发现slice方法很快
    itemRenderList.value = itemSpaces.value.slice(start, end + 1)
    console.log('Log-- ', itemRenderList.value, itemSpaces.value, Math.max(...columnsTop.value), columnsTop.value, 'itemRenderList.value');
}

// 虚拟列表逻辑：需要渲染的items
const itemRenderList = ref([] as any[])
